...
export class MailsClient extends MailsApiClient {
  async waitForMail(address: string, timeoutMs = 60_000, intervalMs = 3_000): Promise<MailDetail> {
    ...
    const knownIds = new Set(initial.data.items.map((item) => item.id))

    while (Date.now() - start < timeoutMs) {
      const current = await this.getMailList(address, 1, 50)
      const nextMail = current.data.items.find((item) => !knownIds.has(item.id)) ?? null

      if (nextMail) {
        const detail = await this.getMail(nextMail.id)
        return detail.data
      }
